...
class MockLLMProvider(BaseLLMProvider):
    ...
    def _extract_patient_context(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for msg in history:
            ...
            if msg.get("role") == "system" and "Patient Context Summary:" in content:
                try:
                    json_part = content.split("Patient Context Summary:\n")[1].split("\nClinical Evidence")[0].strip()
    ...
    def generate(self, question: str, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Route single-patient Clinical Copilot queries to ExplainableReasoningService
        if not snapshot and "[role:" not in q and ("conversation_history" in context or "vitals" in context or "patient_id" in context):
            patient_context = self._extract_patient_context(context)
            explanation = ExplainableReasoningService.process_explainable_query(question, patient_context)
